...
<tr>
  ...
  <th class="group-header" colspan="3" style="background:#e3f2fd">View / Explore</th>
  <th class="group-header" colspan="3" style="background:#c8e6c9">T3 — Justified Edits</th>
...
**Compare T2/T3** = side-by-side view of analysis-ready data vs. your filtered sandbox — not a T3 editing feature, hence grouped under View/Explore.
:::
...
Stored as `sparmvet_decision_hash` in the **Parquet metadata block** — readable without scanning data rows.

**Auto-invalidation:** recipe changes → hash changes → cache stale → recompute from raw.

*Note: this is a **recipe hash** (decisions), not a data content hash. Data integrity verification is not currently implemented.*
:::
...
- Staleness check in microseconds (metadata read, no data scan)
...
```{mermaid}
%%{init: {'theme': 'dark', 'themeVariables': {'fontSize': '14px', 'lineColor': '#a0c4ff'}}}%%
graph LR
